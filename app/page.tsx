...
export default function TodoApp() {
  ...
  const saveCompletionHistory = (currentTodos: Todo[], date: string) => {
    const completed = currentTodos.filter((t) => t.completed).map((t) => t.title)
    const incomplete = currentTodos.filter((t) => !t.completed).map((t) => t.title)

    const storedHistory = localStorage.getItem("completionHistory")
    const existingHistory: DayCompletion[] = storedHistory ? JSON.parse(storedHistory) : []

    // Check if entry for this date already exists
    const existingEntryIndex = existingHistory.findIndex((h) => h.date === date)
  ...
  const addTodo = (todo: Omit<Todo, "id" | "completed" | "createdAt" | "completedDates">) => {
    const newTodo: Todo = {
      ...todo,
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-balance text-3xl font-bold tracking-tight text-foreground">Daily Focus</h1>
              <p className="text-sm text-muted-foreground">Track your recurring tasks and build consistency</p>
            </div>
          </div>
