...
import type { Todo, DayCompletion } from "@/app/page"
...
export function Analytics({ todos, history }: AnalyticsProps) {
  const stats = useMemo(() => {
    ...
    const weeklyCompleted = last7Days.reduce((sum, day) => sum + day.completed.length, 0)
    const weeklyTotal = last7Days.reduce((sum, day) => sum + day.completed.length + day.incomplete.length, 0)
    ...
    for (let i = history.length - 1; i >= 0; i--) {
      const day = history[i]
      const completionRate = day.completed.length / (day.completed.length + day.incomplete.length)
    ...
    const bestDay = history.reduce((best, day) => {
      const rate = day.completed.length / (day.completed.length + day.incomplete.length)
      const bestRate = best.completed.length / (best.completed.length + best.incomplete.length)
    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="border-border/50 bg-gradient-to-br from-blue-500/10 to-cyan-500/10">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">{stats.todayPercentage}%</div>
        ...
        <Card className="border-border/50 bg-gradient-to-br from-emerald-500/10 to-green-500/10">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">{stats.weeklyPercentage}%</div>
        ...
        <Card className="border-border/50 bg-gradient-to-br from-orange-500/10 to-amber-500/10">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Days with 50%+ completion</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-border/50 bg-gradient-to-br from-purple-500/10 to-pink-500/10">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Active recurring tasks</p>
          </CardContent>
        </Card>
      </div>
            ...
            <div className="space-y-3">
              {history
                ...
                .map((day) => {
                  ...
                  const percentage = total > 0 ? Math.round((day.completed.length / total) * 100) : 0
                  ...
                  return (
                    <div key={day.date} className="space-y-2">
                      ...
                      {day.completed.length > 0 && (
                        <div className="text-xs text-muted-foreground">Completed: {day.completed.join(", ")}</div>
                      )}
                    </div>
                  )
                })}
