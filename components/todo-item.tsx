  ...
  return (
    <div className="group flex items-center gap-3 rounded-lg border border-border/50 bg-card p-4 transition-colors hover:border-border hover:bg-accent/50">
      ...
      <div className="flex-1 min-w-0">
        <h4 className={`text-sm font-medium text-foreground ${todo.completed ? "line-through opacity-50" : ""}`}>
          {todo.title}
        </h4>
        {todo.description && (
          <p className={`text-xs text-muted-foreground ${todo.completed ? "line-through opacity-50" : ""}`}>
            {todo.description}
          </p>
        )}
      </div>
